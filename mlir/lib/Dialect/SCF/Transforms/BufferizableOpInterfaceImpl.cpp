...
mergeLoopIterArgMemRefTypes(Operation *loopOp, BufferLikeType initType,
  ...
  if (memref::CastOp::areCastCompatible(initMemRef, yieldMemRef) &&
    ...
    llvm::errs() << "Buffer type mismatch for iter_arg: init_arg buffer type: "
                 << initMemRef << ", yielded value buffer type: " << yieldMemRef
                 << "\n";
...
static FailureOr<BufferLikeType> computeLoopRegionIterArgBufferType(
  ...
  llvm::errs() << "Buffer type mismatch for iter_arg '" << iterArg
               << "': init_arg buffer type: " << *initArgBufferType
               << ", yielded value buffer type: " << yieldedValueBufferType
               << "\n";
  ...
  // Keep the built-in fallback if merge/layout construction did not converge.
  return cast<BufferLikeType>(
      getMemRefTypeWithFullyDynamicLayout(iterTensorType, memorySpace));
}
